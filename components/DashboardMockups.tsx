...
export function EnterpriseDashboard() {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: shouldReduceMotion ? 0 : 40, rotateY: shouldReduceMotion ? 0 : -8 }} animate={{ opacity: 1, x: 0, rotateY: 0 }} transition={{ duration: shouldReduceMotion ? 0 : .9 }} className="relative mx-auto w-full max-w-2xl rounded-[2rem] border border-white/80 bg-white/75 p-3 shadow-glow backdrop-blur-2xl">
      <div className="rounded-[1.5rem] border border-slate-100 bg-slate-950/[.02] p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-[1.2fr_.8fr]">
          <div className="rounded-2xl bg-white p-4 shadow-card"><div className="flex h-40 items-end gap-3">{bars.map((h,i)=><motion.div key={i} initial={{height: shouldReduceMotion ? h + "%" : 20}} animate={{height:h+"%"}} transition={shouldReduceMotion ? { duration: 0 } : {duration:.8,delay:i*.08,repeat:Infinity,repeatType:"reverse",repeatDelay:2}} className="flex-1 rounded-t-xl bg-gradient-to-t from-brand-600 to-violet/70" />)}</div></div>
      ...
      <motion.div animate={shouldReduceMotion ? { y: 0 } : { y: [-8, 8, -8] }} transition={shouldReduceMotion ? { duration: 0 } : { duration: 5, repeat: Infinity }} className="absolute -left-8 top-16 hidden rounded-2xl bg-white p-4 shadow-card md:block"><p className="text-xs text-muted">AI Forecast</p><p className="font-black text-brand-600">+18% capacity</p></motion.div>
      <motion.div animate={shouldReduceMotion ? { y: 0 } : { y: [8, -8, 8] }} transition={shouldReduceMotion ? { duration: 0 } : { duration: 4, repeat: Infinity }} className="absolute -right-6 bottom-16 hidden rounded-2xl bg-white p-4 shadow-card md:block"><p className="text-xs text-muted">Risk</p><p className="font-black text-amber-600">Low attrition</p></motion.div>
    </motion.div>
  );
}
